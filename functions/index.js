...
exports.onDisasterRequestUpdated = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (beforeData.status !== afterData.status) {
      console.log(`Disaster request ${event.params.requestId} status changed from ${beforeData.status} to ${afterData.status}`);
      ...
      try {
        ...
        if (!userQuery.empty) {
          ...
          const statusMessages = {
            'assigned': 'Your emergency report has been assigned to emergency responders.',
            'in-progress': 'Emergency responders are currently handling your reported incident.',
          ...
          const notificationData = {
            ...
            body: statusMessages[afterData.status] || `Status updated to: ${afterData.status}`,
            data: {
              requestId: event.params.requestId,
  ...
  async (request) => {
    ...
    try {
      ...
      await db.collection('news_alerts').add({
        ...
        content: 'This is a test notification from the disaster management system.',
        type: 'info',
        priority: 'low',
...
exports.getEmergencyStats = onCall(
  { cors: true },
  async (request) => {
    const { auth } = request;
    ...
    try {
      ...
      // Get today's stats
      const todayQuery = await db.collection('disaster_requests')
        .where('createdAt', '>=', startOfDay)
        .get();

      // Get this week's stats
      const weekQuery = await db.collection('disaster_requests')
