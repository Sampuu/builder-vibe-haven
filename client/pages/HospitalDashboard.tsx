...
import { Building2, MapPin, Package, RefreshCw, Navigation, Clock, AlertTriangle, Truck, Plus } from 'lucide-react';

export default function HospitalDashboard() {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { user } = useAuth();
  const { supplyRequests, loading, error, createSupplyRequest, updateSupplyRequest, refresh } = useSupplyRequests();
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'assigned': return 'bg-emergency-info';
      case 'in-transit': return 'bg-emergency-info';
      case 'delivered': return 'bg-emergency-resolved';
          ...
          <Card>
            <CardContent className="p-6 text-center">
              <div className="text-2xl font-bold text-emergency-resolved">
                {supplyRequests.filter(r => r.status === 'delivered' && 
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span className="flex items-center">
                ...
                Active Supply Requests ({supplyRequests.filter(r => r.status !== 'delivered').length})
              </span>
              <div className="flex space-x-2">
                <Dialog open={newRequestOpen} onOpenChange={setNewRequestOpen}>
                  ...
                  <DialogContent>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex space-x-2">
                        <Button onClick={handleCreateSupplyRequest} className="flex-1">
                          Create Request
                        </Button>
                        <Button onClick={() => setNewRequestOpen(false)} variant="outline" className="flex-1">
                          Cancel
                        </Button>
                      </div>
                    </div>
                ...
                <Button variant="success" size="sm" onClick={() => navigate('/hospital/supplies')}>
                ...
                {supplyRequests.slice(0, 5).map((request) => (
                  <div key={request.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <div className="font-medium text-slate-900">{request.itemName} (x{request.quantity})</div>
                        <div className="text-sm text-slate-600 flex items-center">
                          <MapPin className="h-4 w-4 mr-1" />
                          {request.location}
                        </div>
                        {request.notes && (
                          <div className="text-xs text-slate-500 mt-1">{request.notes}</div>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      {request.status === 'pending' && (
                        <Dialog open={assignVehicleOpen && selectedRequest === request.id} onOpenChange={(open) => {
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Select a vehicle for delivery of {request.itemName}
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4">
                              {vehicles.map((vehicle) => (
                                <div key={vehicle.id} className="flex items-center justify-between p-3 border rounded">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => handleAssignVehicle(request.id, vehicle.id)}
                                  >
                                    {vehicle.available ? 'Assign' : 'Unavailable'}
                                  </Button>
                                </div>
                              ))}
                            </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <Card className="hover:shadow-lg transition-shadow cursor-pointer">
            <CardHeader className="text-center">
              ...
              <CardDescription>View and manage all supply requests</CardDescription>
            </CardHeader>
            <CardContent>
              <Button className="w-full" variant="info" onClick={() => navigate('/hospital/supplies')}>
                <Package className="mr-2 h-4 w-4" />
                View All Requests
              </Button>
            </CardContent>
          ...
          <Card className="hover:shadow-lg transition-shadow cursor-pointer">
            <CardHeader className="text-center">
              ...
              <CardDescription>Manage hospital transport vehicles</CardDescription>
            </CardHeader>
            <CardContent>
              <Button className="w-full" variant="success" onClick={() => navigate('/hospital/supplies')}>
                <Truck className="mr-2 h-4 w-4" />
                Manage Fleet
              </Button>
            </CardContent>
          ...
          <Card className="hover:shadow-lg transition-shadow cursor-pointer">
            <CardHeader className="text-center">
              ...
              <CardDescription>View all supply locations on map</CardDescription>
            </CardHeader>
            <CardContent>
              <Button className="w-full" variant="warning" onClick={() => navigate('/user/map')}>
