...
export default function RealTimeAlerts({
  ...
  useEffect(() => {
    ...
    if (showDisasterRequests) {
      unsubscribeDisasters = firestoreService.subscribeToDisasterRequests(
        (requests) => {
          ...
          requests.forEach((request) => {
            if (
              ...
              !dismissedAlerts.has(request.id!)
            ) {
    ...
    if (showNewsAlerts) {
      unsubscribeNews = firestoreService.subscribeToNews((news) => {
        const relevantNews = news
          .filter((alert) => alert.targetRoles.includes(user?.role || "user"))
          .slice(0, maxAlerts);
  ...
  const visibleDisasterRequests = disasterRequests.filter(
    (req) => !dismissedAlerts.has(req.id!) && req.status === "pending",
  );

  const visibleNewsAlerts = newsAlerts.filter(
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* No Alerts Message */}
      {visibleDisasterRequests.length === 0 &&
        visibleNewsAlerts.length === 0 && (
          <Card>
