...
export default function MapSystem({
  ...
  const calculateRoute = async (destination: string) => {
    ...
    try {
      ...
      const startCoords = { lat: 28.6139, lng: 77.209 };
  ...
  const geocodeDestination = async (
    ...
    try {
      const response = await fetch(
        `/api/geocode?query=${encodeURIComponent(destination)}`,
        {
          headers: {
            "x-user-id": user?.id || "",
            "x-user-role": user?.role || "user",
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Badge variant="outline" className="flex items-center">
                ...
                {
                  trackedEntities.filter((e) => e.status !== "offline").length
                }{" "}
                Units Online
              </Badge>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                  <div>
                    ...
                    <div className="font-medium">
                      {(currentRoute.route.distance / 1000).toFixed(1)} km
                    </div>
                  </div>
                  <div>
                    <span className="text-blue-600">Duration:</span>
                    <div className="font-medium">
                      {Math.round(currentRoute.route.duration / 60)} min
                    </div>
                  </div>
                  <div>
                    <span className="text-blue-600">Avoided Zones:</span>
                    <div className="font-medium">
                      {currentRoute.avoidedZones.length}
                    </div>
                  </div>
                  <div>
      ...
      >
        ...
        >
          ...
          <MapController
            center={mapCenter}
            zoom={mapZoom}
            routeCoordinates={currentRoute?.route.coordinates.map((coord) => [
              coord[1],
              coord[0],
            ])}
          />
