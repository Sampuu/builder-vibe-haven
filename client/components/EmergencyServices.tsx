...
export default function EmergencyServices({
  ...
  const [filterType, setFilterType] = useState<'all' | 'hospital' | 'police' | 'fire' | 'ambulance'>('all');
  ...
  const filteredEntities = emergencyEntities
    ...
    .map(entity => ({
      ...entity,
      distance: userLocation ? calculateDistance(userLocation, entity.coordinates) : 0
    }))
  ...
  const sortedIncidents = currentIncidents
    .map(incident => ({
      ...incident,
      distance: userLocation ? calculateDistance(userLocation, incident.coordinates) : 0
    }))
    .sort((a, b) => {
  ...
  const getEntityIcon = (type: string) => {
    switch (type) {
      ...
      case 'police': return <ShieldCheck className="h-4 w-4" />;
      case 'fire': return <Flame className="h-4 w-4" />;
      case 'ambulance': return <TruckIcon className="h-4 w-4" />;
