...
const TEST_SCENARIOS: TestScenario[] = [
  {
    ...
    description:
      "Large fire reported at residential building with people trapped on upper floors",
...
export default function NotificationSystemTest() {
  ...
  const runSingleTest = async (scenario: TestScenario) => {
    ...
    try {
      ...
      console.log(
        `📋 Expected departments: ${scenario.expectedDepartments.join(", ")}`,
      );
      ...
      console.log(
        `✅ Notifications sent to: ${notification.targetDepartments.join(", ")}`,
      );

      // Step 3: Verify routing
      const expectedRouting = getNotificationRouting(
        scenario.type,
        scenario.severity,
      );
      ...
      const routingMatch =
        expectedRouting.length === actualRouting.length &&
        ...
        <CardContent className="space-y-4">
          <Alert className="border-blue-200 bg-blue-50">
            ...
            <AlertDescription className="text-blue-800">
              This testing tool simulates emergency incidents and verifies that
              notifications are sent to the correct departments.
            </AlertDescription>
          </Alert>
                ...
                >
                  ...
                  {result.success ? (
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {result.actualDepartments.map((dept: string) => (
                            ...
                            >
                              ...
                              <span>
                                {DEPARTMENT_CONTACTS[dept]?.name || dept}
                              </span>
                            </Badge>
                          ))}
                        </div>
                      ...
                      <div className="text-sm">
                        <span
                          className={`font-medium ${result.routingCorrect ? "text-green-700" : "text-orange-700"}`}
                        >
                          Routing:{" "}
                          {result.routingCorrect ? "CORRECT" : "MISMATCH"}
                        </span>
                      </div>
                    </div>
                  ) : (
