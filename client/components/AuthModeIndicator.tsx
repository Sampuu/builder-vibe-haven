...
export default function AuthModeIndicator({
  showOnlyIfFallback = false,
}: AuthModeIndicatorProps) {
  const [authMode, setAuthMode] = useState<"firebase" | "fallback" | "loading">(
    "loading",
  );
  ...
  return (
    <Alert
      className={`mb-4 ${isFirebaseMode ? "border-green-200 bg-green-50" : "border-blue-200 bg-blue-50"}`}
    >
        ...
        <AlertDescription className="ml-2">
          {isFirebaseMode ? (
            <span className="text-green-800">
              <strong>🔥 Firebase Mode:</strong> Your account data is securely
              stored in Firebase.
            </span>
          ) : (
            <span className="text-blue-800">
              <strong>📱 Demo Mode:</strong> Using local storage. Your data is
              only saved in this browser.
              <br />
              <span className="text-sm mt-1 block">
                To use Firebase, add your Firebase configuration to the .env
                file.
                <a
