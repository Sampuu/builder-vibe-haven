...
export const createIncident = async (
  ...
  const assignedDepartments = INCIDENT_DEPARTMENT_MAPPING[
    incidentData.type
  ] || ["admin"];
...
export const createHelpRequest = async (
  helpData: Omit<HelpRequest, "id" | "timestamps">,
): Promise<HelpRequest> => {
  ...
  localStorage.setItem(
    ...
    JSON.stringify(localHelpRequests),
  );
...
export const updateIncidentStatus = async (
  ...
  const timestampField =
    `timestamps.${status.replace("-", "")}` as keyof Incident["timestamps"];
  ...
  if (isFirebaseAvailable() && db) {
    try {
      ...
      if (updateMessage && userId && userRole) {
        ...
        updateData.updates = [
          ...(await getIncidentUpdates(incidentId)),
          update,
        ];
      }
  ...
  const incidents = JSON.parse(
    localStorage.getItem("emergency-incidents") || "[]",
  );
  const incidentIndex = incidents.findIndex(
    (i: Incident) => i.id === incidentId,
  );

  if (incidentIndex !== -1) {
    incidents[incidentIndex].status = status;
    incidents[incidentIndex].timestamps[
      status.replace("-", "") as keyof Incident["timestamps"]
    ] = timestamp;
 ...
 */
export const getIncidentsForDepartment = async (
  department: UserRole,
): Promise<Incident[]> => {
  if (isFirebaseAvailable() && db) {
    try {
  ...
  return departmentIncidents.sort(
    (a, b) =>
      new Date(b.timestamps.reported).getTime() -
      new Date(a.timestamps.reported).getTime(),
  );
};
 ...
 */
export const getHelpRequestsForDepartment = async (
  department: UserRole,
): Promise<HelpRequest[]> => {
  ...
  if (isFirebaseAvailable() && db) {
    try {
      const q = query(
        collection(db, "help-requests"),
        orderBy("timestamps.requested", "desc"),
      );
      const querySnapshot = await getDocs(q);
  ...
  const requests = JSON.parse(
    localStorage.getItem("emergency-help-requests") || "[]",
  ) as HelpRequest[];
  return requests.sort(
    (a, b) =>
      new Date(b.timestamps.requested).getTime() -
      new Date(a.timestamps.requested).getTime(),
  );
};
