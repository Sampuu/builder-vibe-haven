...
const locationToGeoPoint = (location: { latitude: number; longitude: number }) => {
...
export class BaseDatabaseService {
  protected static async create<T>(collectionPath: string, data: Omit<T, 'id'>): Promise<string> {
    try {
  ...
  protected static async get<T>(collectionPath: string, docId: string): Promise<T | null> {
    try {
  ...
  protected static async update<T>(collectionPath: string, docId: string, data: Partial<T>): Promise<void> {
    try {
      const docRef = doc(db, collectionPath, docId);
...
export class UserService extends BaseDatabaseService {
  ...
  static async createDisasterReport(userId: string, reportData: CreateEmergencyReportForm, userName: string): Promise<string> {
  ...
  static async getUserDisasterReports(userId: string): Promise<UserReportDisaster[]> {
    const userDocRef = doc(db, 'users', userId);
    const reportsCollectionRef = collection(userDocRef, 'reportDisaster');
  ...
  static async createHelpRequest(userId: string, requestData: CreateHelpRequestForm, userName: string): Promise<string> {
    const userDocRef = doc(db, 'users', userId);
  ...
  static async createDisasterNews(userId: string, newsData: CreateNewsForm, authorName: string): Promise<string> {
    const userDocRef = doc(db, 'users', userId);
    ...
    const docRef = await addDoc(newsCollectionRef, {
      ...newNews,
      location: newsData.location ? locationToGeoPoint(newsData.location) : null,
      timestamp: serverTimestamp()
    });
  ...
  static async getUserDisasterNews(userId: string): Promise<UserDisasterNews[]> {
    const userDocRef = doc(db, 'users', userId);
    const newsCollectionRef = collection(userDocRef, 'disasterNews');
...
export class PoliceService extends BaseDatabaseService {
  // Police Reports
  static async createPoliceReport(officerId: string, reportData: Partial<PoliceReport>): Promise<string> {
    const policeDocRef = doc(db, 'police', officerId);
    const reportsCollectionRef = collection(policeDocRef, 'policeReports');
    
    const docRef = await addDoc(reportsCollectionRef, {
      ...reportData,
      location: reportData.location ? locationToGeoPoint(reportData.location) : null,
      timestamp: serverTimestamp()
    });
  ...
  // Case Updates
  static async createCaseUpdate(officerId: string, updateData: Partial<PoliceCaseUpdate>): Promise<string> {
    const policeDocRef = doc(db, 'police', officerId);
  ...
  static async getCaseUpdates(officerId: string, reportId?: string): Promise<PoliceCaseUpdate[]> {
    const policeDocRef = doc(db, 'police', officerId);
    const updatesCollectionRef = collection(policeDocRef, 'caseUpdates');
    
    let q = query(updatesCollectionRef, orderBy('timestamp', 'desc'));
    if (reportId) {
      q = query(updatesCollectionRef, where('reportId', '==', reportId), orderBy('timestamp', 'desc'));
    }
...
export class AmbulanceService extends BaseDatabaseService {
  // Ambulance Requests
  static async createAmbulanceRequest(paramedicId: string, requestData: Partial<AmbulanceRequest>): Promise<string> {
    const ambulanceDocRef = doc(db, 'ambulance', paramedicId);
    const requestsCollectionRef = collection(ambulanceDocRef, 'ambulanceRequests');
    
    const docRef = await addDoc(requestsCollectionRef, {
      ...requestData,
      location: requestData.location ? locationToGeoPoint(requestData.location) : null,
      timestamp: serverTimestamp()
    });
  ...
  static async getAmbulanceRequests(paramedicId: string): Promise<AmbulanceRequest[]> {
    const ambulanceDocRef = doc(db, 'ambulance', paramedicId);
    const requestsCollectionRef = collection(ambulanceDocRef, 'ambulanceRequests');
    const q = query(requestsCollectionRef, orderBy('timestamp', 'desc'));
  ...
  // Dispatch Logs
  static async createDispatchLog(paramedicId: string, logData: Partial<AmbulanceDispatchLog>): Promise<string> {
    const ambulanceDocRef = doc(db, 'ambulance', paramedicId);
  ...
  static async getDispatchLogs(paramedicId: string): Promise<AmbulanceDispatchLog[]> {
    const ambulanceDocRef = doc(db, 'ambulance', paramedicId);
    const logsCollectionRef = collection(ambulanceDocRef, 'dispatchLogs');
...
export class FireBrigadeService extends BaseDatabaseService {
  // Fire Incidents
  static async createFireIncident(firefighterId: string, incidentData: Partial<FireIncident>): Promise<string> {
    ...
    const docRef = await addDoc(incidentsCollectionRef, {
      ...incidentData,
      location: incidentData.location ? locationToGeoPoint(incidentData.location) : null,
      timestamp: serverTimestamp()
    });
  ...
  static async getFireIncidents(firefighterId: string): Promise<FireIncident[]> {
    const fireDocRef = doc(db, 'fireBrigade', firefighterId);
    const incidentsCollectionRef = collection(fireDocRef, 'fireIncidents');
  ...
  // Rescue Logs
  static async createRescueLog(firefighterId: string, logData: Partial<FireRescueLog>): Promise<string> {
    const fireDocRef = doc(db, 'fireBrigade', firefighterId);
  ...
  static async getRescueLogs(firefighterId: string, incidentId?: string): Promise<FireRescueLog[]> {
    const fireDocRef = doc(db, 'fireBrigade', firefighterId);
    const logsCollectionRef = collection(fireDocRef, 'rescueLogs');
    
    let q = query(logsCollectionRef, orderBy('timestamp', 'desc'));
    if (incidentId) {
      q = query(logsCollectionRef, where('incidentId', '==', incidentId), orderBy('timestamp', 'desc'));
    }
...
export class HospitalService extends BaseDatabaseService {
  // Hospital Records
  static async createHospitalRecord(hospitalId: string, recordData: Partial<HospitalRecord>): Promise<string> {
    ...
    const docRef = await addDoc(recordsCollectionRef, {
      ...recordData,
      location: recordData.location ? locationToGeoPoint(recordData.location) : null,
  ...
  static async getHospitalRecords(hospitalId: string): Promise<HospitalRecord[]> {
    const hospitalDocRef = doc(db, 'hospital', hospitalId);
    const recordsCollectionRef = collection(hospitalDocRef, 'hospitalRecords');
  ...
  // Patient Admissions
  static async createPatientAdmission(hospitalId: string, admissionData: Partial<PatientAdmission>): Promise<string> {
    const hospitalDocRef = doc(db, 'hospital', hospitalId);
    const admissionsCollectionRef = collection(hospitalDocRef, 'patientAdmissions');
  ...
  static async getPatientAdmissions(hospitalId: string): Promise<PatientAdmission[]> {
    const hospitalDocRef = doc(db, 'hospital', hospitalId);
    const admissionsCollectionRef = collection(hospitalDocRef, 'patientAdmissions');
...
export class AdminService extends BaseDatabaseService {
  // System Logs
  static async createSystemLog(adminId: string, logData: Partial<SystemLog>): Promise<string> {
  ...
  // Role Management
  static async createRoleManagement(adminId: string, managementData: Partial<RoleManagement>): Promise<string> {
    const adminDocRef = doc(db, 'admin', adminId);
