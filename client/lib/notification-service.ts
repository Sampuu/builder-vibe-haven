...
import { IncidentType, IncidentSeverity, Incident, HelpRequest } from './incident-service';
...
export const getNotificationRouting = (incidentType: IncidentType, severity: IncidentSeverity): UserRole[] => {
...
export const getHelpRequestRouting = (requestType: string, urgency: IncidentSeverity): UserRole[] => {
...
export const sendIncidentNotification = async (incident: Incident): Promise<Notification> => {
  ...
  if (isFirebaseAvailable() && db) {
    try {
      ...
      targetDepartments.forEach(dept => {
        console.log(`📢 ${DEPARTMENT_CONTACTS[dept]?.icon} ${DEPARTMENT_CONTACTS[dept]?.name}: ${notification.title}`);
      });
  ...
  const localNotification = { ...notification, id: `local-notif-${Date.now()}` };
  localNotifications.push(localNotification);
  localStorage.setItem('emergency-notifications', JSON.stringify(localNotifications));
 ...
 */
export const sendHelpRequestNotification = async (helpRequest: HelpRequest): Promise<Notification> => {
  const targetDepartments = getHelpRequestRouting(helpRequest.type, helpRequest.urgency);
  ...
  if (isFirebaseAvailable() && db) {
    try {
      ...
      console.error('Failed to send help request notification via Firebase:', error);
    }
  }

  // Fallback to local storage
  const localNotification = { ...notification, id: `local-help-notif-${Date.now()}` };
  localNotifications.push(localNotification);
  localStorage.setItem('emergency-notifications', JSON.stringify(localNotifications));
  
  console.log('📱 Help request notification sent locally to:', targetDepartments.join(', '));
  targetDepartments.forEach(dept => {
    console.log(`📢 ${DEPARTMENT_CONTACTS[dept]?.icon} ${DEPARTMENT_CONTACTS[dept]?.name}: ${notification.title}`);
  });
 ...
 */
export const getNotificationsForDepartment = async (department: UserRole): Promise<Notification[]> => {
  if (isFirebaseAvailable() && db) {
    try {
      const q = query(
      ...
      return notifications.sort((a, b) => 
        new Date(b.timestamps.sent).getTime() - new Date(a.timestamps.sent).getTime()
      );
    } catch (error) {
  ...
  // Fallback to local storage
  const notifications = JSON.parse(localStorage.getItem('emergency-notifications') || '[]') as Notification[];
  const departmentNotifications = notifications.filter(notification =>
    notification.targetDepartments.includes(department)
  );
  
  return departmentNotifications.sort((a, b) =>
    new Date(b.timestamps.sent).getTime() - new Date(a.timestamps.sent).getTime()
  );
};
...
export const acknowledgeNotification = async (
  ...
  if (isFirebaseAvailable() && db) {
    try {
      ...
      if (notification) {
        const updatedAcknowledgments = [...notification.acknowledgments, acknowledgment];
        await updateDoc(notificationRef, {
          acknowledgments: updatedAcknowledgments,
          isRead: true
        });
        console.log('🔥 Notification acknowledged in Firebase:', notificationId);
      }
      return;
    } catch (error) {
  ...
  if (notificationIndex !== -1) {
    ...
    localStorage.setItem('emergency-notifications', JSON.stringify(notifications));
...
export const getNotificationStats = async (department: UserRole) => {
  ...
  const stats = {
    ...
    byType: {
      ...
      help_request: notifications.filter(n => n.type === 'help_request').length,
...
export const sendStatusUpdateNotification = async (
  ...
  // Fallback to local storage
  const localNotification = { ...notification, id: `local-status-${Date.now()}` };
  localNotifications.push(localNotification);
  localStorage.setItem('emergency-notifications', JSON.stringify(localNotifications));
