 ...
 */
export function getEmergencyContact(incidentType: string): {
  number: string;
  label: string;
} {
  switch (incidentType) {
...
export function estimateResponseTime(
  ...
  const severityMultiplier =
    severity === "critical" ? 0.7 : severity === "high" ? 0.8 : 1.0;
...
export function sortIncidentsByPriority(
  incidents: Array<{ severity?: string; timestamp?: Date }>,
): Array<{ severity?: string; timestamp?: Date }> {
  return incidents.sort((a, b) => {
    const priorityA = getIncidentPriority(
      a.severity || "low",
      a.timestamp || new Date(),
    );
    const priorityB = getIncidentPriority(
      b.severity || "low",
