...
class DashboardService {
  ...
  public connect(user: { id: string; role: string; name: string }) {
    ...
    console.log(`🔌 Connecting dashboard service for ${user.name} (${user.role})`);
  }
  ...
  public requestBackup(request: Omit<BackupRequest, 'id' | 'timestamp'>) {
    ...
    console.log(`🚨 Backup request sent to ${request.targetRole}:`, request.reason);
  }

  public updateIncidentStatus(incidentId: string, status: string, additionalData?: any) {
    ...
    if (callbacks) {
      callbacks.forEach(callback => {
        try {
          ...
          console.error(`Error in dashboard service callback for ${event}:`, error);
        }
      });
    }
  }
