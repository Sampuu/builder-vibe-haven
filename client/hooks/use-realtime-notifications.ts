...
export const useRealtimeNotifications = () => {
  ...
  const addNotification = useCallback(
    (
      report: AnyReport,
      type: "emergency" | "high_priority" | "general" = "general",
    ) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeHighPriority = createHighPriorityListener((reports) => {
      ...
      const newReports = reports.filter(
        (report) => new Date(report.timestamp) > oneMinuteAgo,
      );
    ...
    const unsubscribeEmergency = createEmergencyNotificationListener(
      (reports) => {
