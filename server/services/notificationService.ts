...
export function sendIncidentNotification(incident: Incident): void {
  ...
  const notificationTitle = incident.type === 'help_request' 
    ? 'New Help Request' 
  ...
  const notification = createNotification({
    ...
    type: incident.urgency === 'critical' ? 'emergency' : 
          incident.urgency === 'high' ? 'warning' : 'info',
    priority: incident.urgency === 'critical' || incident.urgency === 'high' ? 'high' : 'medium',
    category: 'incident',
  ...
  console.log(`✅ Sent notification for incident ${incident.id} to departments: ${incident.assignedDepartments.join(', ')}`);
}

export function sendIncidentStatusNotification(incident: Incident, updatedBy?: { userId: string; role: UserRole }): void {
  ...
  console.log(`✅ Sent status notification for incident ${incident.id} (${incident.status})`);
}

export function sendIncidentAcknowledgmentNotification(incident: Incident, department: UserRole): void {
  ...
  console.log(`✅ Sent acknowledgment notification for incident ${incident.id} by ${department}`);
}
...
export function broadcastNewsNotification(
  ...
  console.log(`📢 Broadcast news notification: ${title} (${type}, ${priority})`);
  return notification;
}
...
export function sendTargetedNotification(
  ...
  console.log(`🎯 Sent targeted notification to roles: ${targetRoles.join(', ')}`);
  return notification;
}
