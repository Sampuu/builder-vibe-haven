...
export const createIncident: RequestHandler = (req, res) => {
  try {
    ...
    const reportedByRole =
      (req.headers["x-user-role"] as User["role"]) || "user";
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Missing required fields: type, title, description, location, contactPhone",
      });
    }
    ...
    console.log(
      `Created incident ${incident.id} for roles:`,
      incident.targetRoles,
    );
