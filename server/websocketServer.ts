...
export interface DashboardEvent {
  type: 'incident_created' | 'incident_updated' | 'incident_deleted' | 
        'mission_created' | 'mission_updated' | 'mission_deleted' |
        'backup_requested' | 'status_updated' | 'supply_requested' |
        'notification_created' | 'user_action';
  data: any;
  timestamp: string;
  userId?: string;
  userRole?: string;
...
class DashboardWebSocketServer {
  ...
  private setupEventHandlers() {
    this.io.on('connection', (socket) => {
      ...
      socket.on('join_dashboard', (userData: { id: string; role: string; name: string }) => {
        ...
        console.log(`👤 User ${userData.name} (${userData.role}) joined dashboard`);
      });
      ...
      socket.on('backup_request', (data) => {
        ...
        if (data.targetRole) {
          socket.to(`dashboard_${data.targetRole}`).emit('backup_requested', data);
        }
        // Also send to admin
      ...
      socket.on('send_notification', (data) => {
        if (data.targetUserId) {
          ...
          socket.to(`dashboard_${data.targetRole}`).emit('notification_received', data);
        } else {
          // Broadcast to all
    ...
    switch (event.type) {
      ...
      case 'backup_requested':
        ...
        if (event.data.targetRole) {
          socket.to(`dashboard_${event.data.targetRole}`).emit('dashboard_event', event);
        }
  ...
  private logEvent(eventType: string, data: any, socket: any) {
    const user = this.connectedUsers.get(socket.id);
    console.log(`📡 Dashboard Event [${eventType}] from ${user?.name || 'unknown'} (${user?.role || 'unknown'}):`, data);
  }
