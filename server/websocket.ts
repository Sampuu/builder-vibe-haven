...
export function setupWebSocket(server: Server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('authenticate', (data: { userId: string; userRole: User['role'] }) => {
      ...
      console.log(`User ${data.userId} (${data.userRole}) authenticated and joined role room`);
    });
...
export function broadcastNotification(notification: Notification) {
  ...
  console.log(`Broadcasted notification ${notification.id} to role: ${notification.targetRole}`);
}

export function broadcastIncidentUpdate(incidentId: string, update: any, targetRoles: User['role'][]) {
  if (!io) {
    console.warn('WebSocket server not initialized');
    return;
  }
  ...
  console.log(`Broadcasted incident update for ${incidentId} to roles:`, targetRoles);
}
